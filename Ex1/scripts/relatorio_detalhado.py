...
def main():
    ...
    p('Comparar quatro implementações seriais de multiplicação de matrizes com dois modos de alocação, medindo tempo, leituras e faltas em L1 de dados, instruções de desvio e erros de predição. O enunciado define oito experimentos; as versões calculam a mesma multiplicação, alterando a ordem e a organização dos laços.')
    table([['Exp.', 'Técnica', 'Alocação', 'Laços / transformação'],
          ...
          [35,90,75,width-200], 'Configurações dos oito experimentos.')
    p('R[i][j] = Σ<sub>k=0</sub><super>N−1</super> A[i][k] × B[k][j]', 'Formula')
    p(f'Foram realizadas <b>{8*data["n"]} execuções válidas</b>, dez por configuração. Todas utilizaram matrizes {data["tamanho"]} × {data["tamanho"]} de <b>float</b>, entradas idênticas e compilação <b>-O0</b>, que mantém desativadas as principais otimizações automáticas do compilador. A melhor média observada foi a do experimento 3: <b>{estimate(3,"tempo_segundos")} s</b> (média ± margem do IC95%).')
    ...
    table([['Item','Configuração registrada'],
          ...
          [95,width-95], 'Ambiente de execução e compilação.')
    ...
    p('<b>Ordem e repetições.</b> Dez execuções de cada experimento, com a ordem E1 a E8 sorteada a cada repetição usando a semente 9512026. Execuções exploratórias anteriores não integram a amostra de 80 medições. Cada observação usa um processo novo, sem chamada de aquecimento interna. A primeira escrita na saída integra a medição. Nenhuma observação definitiva foi descartada.')
    ...
    p('<b>Média e IC95%.</b> Para cada configuração e cada uma das cinco métricas, calculou-se a média, o desvio padrão amostral (denominador n−1) e o intervalo bilateral pela distribuição t de Student.')
    ...
    p('<b>Influências.</b> Foram escolhidos previamente os conjuntos E1,E2,E3,E4 para cache e E1,E2,E5,E6 para branch. Em cada análise 2², A é a técnica (−1 simples, +1 otimizada), B é a alocação (−1 estática, +1 dinâmica), e AB é a interação.')
    ...
    p('A análise foi conferida em R 4.5.3: planejamento 2² explícito, lm, anova e normalização pelas três Mean Sq. As 40 médias, desvios e ICs e as 12 influências de cada normalização coincidiram com Python dentro das tolerâncias registradas. Para o ajuste das quatro médias, há zero graus de liberdade residuais; não se interpretam testes F desse modelo.', 'Pequeno')
    ...
    table(rows,[30,160,175,width-365], 'Tempos médios, IC95% e razão em relação à versão simples.')
    ...
    p(f'Na alocação dinâmica, E8 teve a menor média: {estimate(8,"tempo_segundos")} s, redução de {f(reduction(mean(2,"tempo_segundos"),mean(8,"tempo_segundos")),1)}% ante E2. E6 teve média {f(100*(mean(6,"tempo_segundos")/mean(2,"tempo_segundos")-1),1)}% maior que E2.')
    ...
    table([['Exp.','L1 loads (milhões), média ± IC95%','L1 misses (milhões), média ± IC95%']] +
          ...
          [35,(width-35)/2,(width-35)/2], 'Contadores de cache: médias e margens dos IC95%.')
    ...
    table([['Exp.','Branch instr. (milhões), média ± IC95%','Branch misses (milhares), média ± IC95%']] +
          [[str(e),estimate(e,'branch-instructions',1e6,6),estimate(e,'branch-misses',1e3,3)] for e in range(1,9)],
          [35,(width-35)/2,(width-35)/2], 'Contadores de desvios: médias e margens dos IC95%.')
    ...
    table(rows,[105,100,100,100,width-405], 'Percentuais da soma de quadrados total das 40 observações de cada subconjunto. Valores exibidos como 0,000% podem ser positivos abaixo da precisão de apresentação.')
    ...
    table(rows,[130,(width-130)/3,(width-130)/3,(width-130)/3], 'Normalização entre as quatro médias, sem componente de erro. Valores conferidos por lm e anova em R; cada fator tem um grau de liberdade, portanto Mean Sq = Sum Sq.')
    p(f'<b>Cache (E1 a E4).</b> A alocação explica {f(effects[metrics[0]]["fatores"]["alocacao"]["influencia_pct_total"],3)}% da variação total em loads; a técnica explica {f(effects[metrics[1]]["fatores"]["tecnica"]["influencia_pct_total"],3)}% em misses. Em média sobre as duas técnicas, mudar de estática para dinâmica acrescentou {f(effects[metrics[0]]["fatores"]["alocacao"]["efeito"]/1e6,2)} milhões de loads. Interchange reduziu, em média sobre as alocações, {f(-effects[metrics[1]]["fatores"]["tecnica"]["efeito"]/1e6,2)} milhões de misses.')
    p(f'<b>Branch (E1,E2,E5,E6).</b> A técnica responde por praticamente 100% da variação nas instruções de desvio. Já em branch misses, <b>{f(effects[metrics[3]]["residuo_pct_total"],2)}% é resíduo entre repetições</b>. O efeito médio do unrolling foi de apenas {f(effects[metrics[3]]["fatores"]["tecnica"]["efeito"],1)} erros por execução. As diferenças entre as quatro médias são pequenas em relação ao total de erros de predição.')

    page()
    title('8. Efeitos principais e interação: cache')
    figure('efeitos_interacoes_cache', 'Figura 5: E1 a E4. À esquerda e ao centro: médias marginais sobre o outro fator. À direita: médias de cada combinação com IC95%. Os eixos verticais têm escalas próprias.', 355)
    p('Os gráficos mostram os efeitos principais e a interação, complementando os percentuais das Tabelas 6 e 7. Cada ponto corresponde aos níveis da técnica e da alocação indicados nos eixos.')
    p('Em L1 loads, a linha do efeito de alocação apresenta a maior mudança: o acesso dinâmico exige mais leituras. No gráfico de interação, a distância entre as duas alocações é quase constante entre simples e interchange, coerente com o componente de interação muito pequeno.')
    p('Em L1 misses, ambas as alocações apresentam uma queda acentuada com interchange. As diferenças entre estática e dinâmica são maiores no caso simples e se tornam pequenas após a troca dos laços. Essa diferença entre efeitos condicionais corresponde ao termo de interação, embora ele represente apenas uma pequena parcela da variação total.')
    p('Os painéis de efeitos principais mostram médias marginais, não o resultado de uma única configuração. Uma linha visualmente inclinada não implica, por si só, grande efeito: é preciso considerar a escala do eixo e os valores numéricos.')
    ...
    p('Em branch instructions, o efeito do unrolling é grande nas duas alocações. As médias estática e dinâmica praticamente coincidem quando a técnica é fixada. A diferença entre as alocações é pequena em relação às dezenas de milhões de desvios medidos.')
    p(f'Em branch misses, as linhas da interação são quase paralelas: o unrolling diminuiu ligeiramente a média nas duas alocações. As diferenças são pequenas em escala absoluta e seus intervalos apresentam sobreposição. A decomposição com réplicas atribui {f(effects["branch-misses"]["residuo_pct_total"],2)}% da variação total ao resíduo; não se afirma uma melhora geral dos erros de predição.')
    p(f'A Figura 6 ajuda a interpretar por que a interação corresponde a {f(effects["branch-misses"]["fatores"]["interacao"]["influencia_pct_entre_medias"],2)}% quando o denominador inclui apenas a variação entre quatro médias, mas a {f(effects["branch-misses"]["fatores"]["interacao"]["influencia_pct_total"],2)}% quando inclui também a variação entre execuções. Ambos foram mantidos e identificados para evitar confundir influência relativa com tamanho do ganho.')
    ...
    p('Ferramentas utilizadas: perf 7.1.13, NumPy 2.2.6, SciPy 1.14.1 e R 4.5.3. O script em R usa as quatro combinações do planejamento 2² e as funções lm e anova.')
    title('Referências')
    refs = [
        'BRUSCHI, Sarita Mazzini. 3ª aula: Técnicas básicas para otimização de código serial / Profiling. Arquivo local 3aAula_Profiling.pdf, páginas 14 a 17.',
        'Linux perf. perf-stat(1): controle enable/disable, --delay e grupos de eventos. <link href="https://man7.org/linux/man-pages/man1/perf-stat.1.html" color="#174A76">man7.org/linux/man-pages/man1/perf-stat.1.html</link>. Consulta: 07/09/2026.',
        'NIST/SEMATECH. Confidence Limits for the Mean. <link href="https://www.itl.nist.gov/div898/handbook/eda/section3/eda352.htm" color="#174A76">NIST e-Handbook, seção 1.3.5.2</link>. Consulta: 07/09/2026.',
        'NIST/SEMATECH. The two-way ANOVA. <link href="https://itl.nist.gov/div898/handbook/prc/section4/prc427.htm" color="#174A76">NIST e-Handbook, seção 7.4.2.7</link>. Consulta: 07/09/2026.',
        'SSC0951. Script R de apoio para cálculo das influências. Arquivo fornecido pelo grupo: r.txt; cópia preservada em material_apoio/r_original.txt. Conferido em 08/09/2026.',
        'R Core Team. ANOVA for Linear Model Fits. <link href="https://stat.ethz.ch/R-manual/R-devel/library/stats/html/anova.lm.html" color="#174A76">Documentação de anova.lm</link>. Consulta: 08/09/2026.'
    ]
    for ref in refs:
        p(ref,'Pequeno')
    p('<b>Identificação:</b> ' + escape(args.integrantes), 'Pequeno')

    pdf = root.parent / 'entrega/Anexo_tecnico.pdf'
