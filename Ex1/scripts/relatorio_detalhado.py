...
def main():
    ...
    p('A análise foi conferida em R 4.5.3 com uma adaptação do script fornecido em r.txt [6]: planejamento 2² explícito, lm, anova e normalização pelas três Mean Sq. As 40 médias, desvios e ICs e as 12 influências de cada normalização coincidiram com Python dentro das tolerâncias registradas. Para o ajuste das quatro médias, há zero graus de liberdade residuais; não se interpretam testes F desse modelo [7].', 'Pequeno')
    ...
    p('Tabela 6 — Método do script R de apoio: normalização entre as quatro médias, sem componente de erro. Valores conferidos por lm e anova em R; cada fator tem um grau de liberdade, portanto Mean Sq = Sum Sq.', 'Pequeno')
    ...
    refs = [
        ...
        '[7] R Core Team. ANOVA for Linear Model Fits. <link href="https://stat.ethz.ch/R-manual/R-devel/library/stats/html/anova.lm.html" color="#174A76">Documentação de anova.lm</link>. Consulta: 08/09/2026.'
    ]
    for ref in refs:
        p(ref,'Pequeno')
